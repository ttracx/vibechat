...
from vibechat.common import compute_init, compute_cleanup, print0, DummyWandb, print_banner, get_base_dir
from vibechat.tokenizer import get_tokenizer, get_token_bytes
from vibechat.checkpoint_manager import save_checkpoint
from vibechat.loss_eval import evaluate_bpb
...
exec(open(os.path.join('vibechat', 'configurator.py')).read()) # overrides from command line or config file
...
wandb_run = DummyWandb() if use_dummy_wandb else wandb.init(project="vibechat", name=run, config=user_config)
...
# Log to report
from vibechat.report import get_report
